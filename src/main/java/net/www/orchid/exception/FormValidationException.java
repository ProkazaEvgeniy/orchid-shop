package net.www.orchid.exception;
...
public class FormValidationException extends DataAccessException {
	...
	public FieldError buildFieldError(String formName) {
		...
		codes.addAll(Arrays
				.asList(new String[] { FormValidationException.class.getSimpleName() + "." + formName + "." + fieldName,
						formName + "." + fieldName, fieldName }));
		return new FieldError(formName, fieldName, rejectedValue, false, codes.toArray(Constants.EMPTY_ARRAY),
				Constants.EMPTY_ARRAY, toString());
	}

	@Override
	public String toString() {
		return String.format("FormValidationException [fieldName=%s, rejectedValue=%s, errorCode=%s]", fieldName,
				rejectedValue, errorCode);
	}
